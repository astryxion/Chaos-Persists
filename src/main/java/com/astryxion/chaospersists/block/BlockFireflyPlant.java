...
public class BlockFireflyPlant extends CropsBlock {
...
public RenderType getRenderType(BlockState state) {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random rand) {
        ...
        List<Firefly> FireflyList = world.getEntitiesOfClass(Firefly.class, aabb);
        ...
        if (world.getBlockState(pos.above()).getBlock() == Blocks.AIR && world.isDay()) {
            ResourceLocation rl = new ResourceLocation("chaospersists", "firefly");
            EntityType<?> type = ForgeRegistries.ENTITIES.getValue(rl);
            if (type != null) {
                Entity Firefly = type.create(world);
                if (Firefly != null) {
                    Firefly.moveTo(pos.getX(), pos.getY() + 1, pos.getZ(),
                            world.random.nextFloat() * 360.0F, 0.0F);
                    world.addFreshEntity(Firefly);
                }
            }
        }
    }
