...
public class BlockMosquitoPlant extends CropsBlock {
...
public RenderType getRenderType(BlockState state) {
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random rand) {
        ...
        List<EntityMosquito> MosquitoList = world.getEntitiesOfClass(EntityMosquito.class, aabb);
        ...
        if (world.getBlockState(pos.above()).getBlock() == Blocks.AIR && world.isDay()) {
            ResourceLocation rl = new ResourceLocation("chaospersists", "mosquito");
            EntityType<?> type = ForgeRegistries.ENTITIES.getValue(rl);
            if (type != null) {
                Entity Mosquito = type.create(world);
                if (Mosquito != null) {
                    Mosquito.moveTo(pos.getX(), pos.getY() + 1, pos.getZ(),
