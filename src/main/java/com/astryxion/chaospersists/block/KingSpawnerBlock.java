...
public class KingSpawnerBlock extends SugarCaneBlock {

    public KingSpawnerBlock() {
        this(0.9F);
    }

    public KingSpawnerBlock(float lightLevel) {
    ...
    public void tick(BlockState state, ServerWorld world, BlockPos pos, Random rand) {
        if (ChaosPersists.TheKingEnable != 0) {
            spawnTheKing(world, (double) pos.getX(), (double) (pos.getY() + 8), (double) pos.getZ());
        }
    ...
    public static Entity spawnTheKing(World world, double x, double y, double z) {
        EntityType<?> type = ForgeRegistries.ENTITIES.getValue(new ResourceLocation("chaospersists", "the_king"));
        if (type == null) {
            return null;
        }
