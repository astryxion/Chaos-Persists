...
public class CommandDanger {
    public static final String NAME = "danger";
    private static final int DIMENSION_INDEX = 4;
    ...
    public static LiteralArgumentBuilder<CommandSource> build(LiteralArgumentBuilder<CommandSource> builder) {
        return builder.requires(source -> source.hasPermission(2)).executes(CommandDanger::execute);
    }
    ...
    private static int execute(CommandContext<CommandSource> context) {
        ...
        if (world == null) {
            player.sendMessage(new StringTextComponent(TextFormatting.RED + "Danger dimension is not available."), player.getUUID());
            return 0;
        }
        if (player.getLevel().dimension() == world.dimension()) {
            player.sendMessage(new StringTextComponent(TextFormatting.YELLOW + "You are already in the Danger dimension."), player.getUUID());
            return 0;
        }
        ...
        player.sendMessage(new StringTextComponent(TextFormatting.GREEN + "Teleported to Danger."), player.getUUID());
        return 1;
    }
}
