...
public class CommandUtopia {
    ...
    public static LiteralArgumentBuilder<CommandSource> build(LiteralArgumentBuilder<CommandSource> builder) {
        return builder.requires(source -> source.hasPermission(2)).executes(CommandUtopia::execute);
    }
    ...
    private static int execute(CommandContext<CommandSource> context) {
        ...
        if (world == null) {
            player.sendMessage(new StringTextComponent(TextFormatting.RED + "Utopia dimension is not available."), player.getUUID());
            return 0;
        }
        if (player.getLevel().dimension() == world.dimension()) {
            player.sendMessage(new StringTextComponent(TextFormatting.YELLOW + "You are already in the Utopia dimension."), player.getUUID());
            return 0;
        }
        ...
        player.sendMessage(new StringTextComponent(TextFormatting.GREEN + "Teleported to Utopia."), player.getUUID());
        return 1;
    }
}
