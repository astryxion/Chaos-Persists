...
public class CommandVillageMania {
    public static final String NAME = "villagemania";
    private static final int DIMENSION_INDEX = 3;
    ...
    public static LiteralArgumentBuilder<CommandSource> build(LiteralArgumentBuilder<CommandSource> builder) {
        return builder.requires(source -> source.hasPermission(2)).executes(CommandVillageMania::execute);
    }
    ...
    private static int execute(CommandContext<CommandSource> context) {
        ...
        if (world == null) {
            player.sendMessage(new StringTextComponent(TextFormatting.RED + "Village Mania dimension is not available."), player.getUUID());
            return 0;
        }
        if (player.getLevel().dimension() == world.dimension()) {
            player.sendMessage(new StringTextComponent(TextFormatting.YELLOW + "You are already in the Village Mania dimension."), player.getUUID());
            return 0;
        }
        ...
        player.sendMessage(new StringTextComponent(TextFormatting.GREEN + "Teleported to Village Mania."), player.getUUID());
        return 1;
    }
}
