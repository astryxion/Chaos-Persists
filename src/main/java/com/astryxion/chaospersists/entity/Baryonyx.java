...
extends AnimalEntity {
    ...
    public Baryonyx(EntityType<? extends Baryonyx> type, World par1World) {
    ...
    private boolean scan_it(int x, int y, int z, int dx, int dy, int dz) {
        ...
        for (i = - dy; i <= dy; ++i) {
            for (j = - dz; j <= dz; ++j) {
                ...
                if (bid == Blocks.GRASS_BLOCK && (d = dx * dx + j * j + i * i) < this.closest) {
                    this.closest = d;
                    this.tx = x + dx;
                    this.ty = y + i;
                    this.tz = z + j;
                ...
                if ((bid = this.level.getBlockState(new net.minecraft.util.math.BlockPos(x - dx, y + i, z + j)).getBlock()) != Blocks.GRASS_BLOCK || (d = dx * dx + j * j + i * i) >= this.closest) continue;
                this.closest = d;
                this.tx = x - dx;
                this.ty = y + i;
                this.tz = z + j;
        ...
        for (i = - dx; i <= dx; ++i) {
            for (j = - dz; j <= dz; ++j) {
                ...
                if (bid == Blocks.GRASS_BLOCK && (d = dy * dy + j * j + i * i) < this.closest) {
                    this.closest = d;
                    this.tx = x + i;
                    this.ty = y + dy;
                    this.tz = z + j;
                ...
                if ((bid = this.level.getBlockState(new net.minecraft.util.math.BlockPos(x + i, y - dy, z + j)).getBlock()) != Blocks.GRASS_BLOCK || (d = dy * dy + j * j + i * i) >= this.closest) continue;
                this.closest = d;
                this.tx = x + i;
                this.ty = y - dy;
                this.tz = z + j;
        ...
        for (i = - dx; i <= dx; ++i) {
            for (j = - dy; j <= dy; ++j) {
                ...
                if (bid == Blocks.GRASS_BLOCK && (d = dz * dz + j * j + i * i) < this.closest) {
                    this.closest = d;
                    this.tx = x + i;
                    this.ty = y + j;
                    this.tz = z + dz;
                ...
                if ((bid = this.level.getBlockState(new net.minecraft.util.math.BlockPos(x + i, y + j, z - dz)).getBlock()) != Blocks.GRASS_BLOCK || (d = dz * dz + j * j + i * i) >= this.closest) continue;
                this.closest = d;
                this.tx = x + i;
                this.ty = y + j;
                this.tz = z - dz;
    ...
    protected void customServerAiStep() {
        ...
        if (this.level.random.nextInt(60) == 0 && ChaosPersists.PlayNicely == 0) {
            this.closest = 99999;
            this.tz = 0;
            this.ty = 0;
            this.tx = 0;
            ...
            if (this.closest < 99999) {
                ...
                if (this.closest < 12) {
                    if (this.level.getGameRules().getBoolean(net.minecraft.world.GameRules.RULE_MOBGRIEFING)) {
                        this.level.setBlock(new net.minecraft.util.math.BlockPos(this.tx, this.ty, this.tz), Blocks.DIRT.defaultBlockState(), 2);
                    }
                    this.heal(1.0f);
                    this.playSound(net.minecraft.util.registry.Registry.SOUND_EVENT.get(new net.minecraft.util.ResourceLocation("entity.generic.burp")), 1.0f, this.level.random.nextFloat() * 0.2f + 0.9f);
                }
            }
        }
    }
    ...
    private int findBuddies() {
        List var5 = this.level.getEntitiesOfClass(Baryonyx.class, this.getBoundingBox().inflate(20.0, 10.0, 20.0));
        return var5.size();
    }
}
