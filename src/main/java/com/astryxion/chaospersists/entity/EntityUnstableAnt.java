...
public class EntityUnstableAnt extends EntityAnt {
    public EntityUnstableAnt(EntityType<? extends EntityUnstableAnt> type, World par1World) {
    ...
    public boolean interact(PlayerEntity par1PlayerEntityEntity) {
        ...
        ServerWorld targetWorld = ChaosPersists.getServerWorldByDimensionId(ChaosPersists.getDimension(4));
        ServerWorld overworld = server.getLevel(World.OVERWORLD);
        if (targetWorld == null || overworld == null) {
            return false;
        }
        if (serverPlayerEntity.getLevel() != targetWorld) {
            serverPlayerEntity.changeDimension(targetWorld, (ITeleporter) new ChaosTeleporter(targetWorld, ChaosPersists.getDimension(4), this.level));
        } else {
