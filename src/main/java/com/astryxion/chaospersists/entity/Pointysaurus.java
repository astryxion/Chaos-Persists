...
public class Pointysaurus extends Monster {
    private static final EntityDataAccessor<Byte> ATTACKING =
            SynchedEntityData.defineId(Pointysaurus.class, EntityDataSerializers.BYTE);
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMonsterAttributes()
                .add(Attributes.MAX_HEALTH, (double) ChaosPersists.Pointysaurus_stats.health)
                .add(Attributes.MOVEMENT_SPEED, 0.35)
                .add(Attributes.ATTACK_DAMAGE, (double) ChaosPersists.Pointysaurus_stats.attack)
                .add(Attributes.ARMOR, (double) ChaosPersists.Pointysaurus_stats.defense);
    }
    ...
    public static boolean checkPointysaurusSpawnRules(
            EntityType<Pointysaurus> type,
        ...
        for (int k = -3; k < 3; ++k) {
            for (int j = -3; j < 3; ++j) {
                for (int i = 0; i < 5; ++i) {
                    ...
                    if (id != null && "Pointysaurus".equals(id.getPath())) {
                        return true;
                    }
                }
            }
    ...
    protected boolean isValidLightLevel(LevelAccessor level) {
        if (level instanceof ServerLevelAccessor serverLevel) {
            return Monster.checkMonsterSpawnRules(
                    ChaosPersists.ENTITY_TYPE_POINTYSAURUS.get(),
                    serverLevel,
