...
public class ThePrinceTeen extends TamableAnimal {
    ...
    private static final EntityDataAccessor<Integer> HEAD2_EXT =
            SynchedEntityData.defineId(ThePrinceTeen.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> THE_PRINCE_TEEN_FIRE =
            SynchedEntityData.defineId(ThePrinceTeen.class, EntityDataSerializers.INT);
    ...
    public void travel(Vec3 travelVector) {
        ...
        if (this.isVehicle() && this.getControllingPassenger() instanceof Player pp && this.getActivity() != 0) {
            ...
            if (!this.level().isClientSide
                ...
                this.princeTeenRiderStrafeAttack(pp, mx, my, mz);
            }
            ...
            if (!this.level().isClientSide) {
                List<Entity> list =
                        this.level()
                                .getEntities(
                                        this,
                                        this.getBoundingBox().inflate(3.25, 4.0, 3.25),
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(THE_PRINCE_TEEN_FIRE, 1);
    ...
    public int getThePrinceTeenFire() {
        return this.entityData.get(THE_PRINCE_TEEN_FIRE);
    }
    ...
    public void setThePrinceTeenFire(int par1) {
        ...
        this.entityData.set(THE_PRINCE_TEEN_FIRE, par1);
    }
    ...
    public boolean doHurtTarget(Entity par1Entity) {
        ...
        if (par1Entity instanceof LivingEntity living) {
            ...
            living.hurt(this.damageSources().mobAttack(this), iskraken * 45.0f);
        ...
        if (this.kill_count > 25 && this.day_count > 10) {
            Entity ent =
                    spawnCreature(
                            ...
                            this.getZ());
            if (ent != null) {
                transferTameToAdult(ent, this.isTame() ? this.getOwnerUUID() : null);
                this.discard();
            }
        }
        if (this.is_day == 0) {
        ...
        if (this.getRandom().nextInt(5) == 1 && this.level().getDifficulty() != Difficulty.PEACEFUL) {
            ...
            if (e != null) {
                ...
                if (this.distanceToSqr(e)
                        < (double) ((8.0f + e.getBbWidth() / 2.0f) * (8.0f + e.getBbWidth() / 2.0f))) {
                    this.doHurtTarget(e);
    ...
    private LivingEntity findSomethingToAttack() {
        ...
        List<LivingEntity> var5 =
                this.level()
                        .getEntitiesOfClass(
                                LivingEntity.class, this.getBoundingBox().inflate(25.0, 20.0, 25.0));
            ...
            if (!var2.isEmpty()
                ...
                Entity ent =
                        spawnCreature(
                                ...
                                this.getZ());
                if (ent != null) {
                    transferTameToPrince(ent, par1EntityPlayer.getUUID(), this.isTame());
                    this.discard();
                }
                if (!par1EntityPlayer.getAbilities().instabuild) {
                    var2.shrink(1);
    ...
    private static void transferTameToPrince(Entity ent, UUID ownerUuid, boolean fromTeenTamed) {
        if (ent instanceof ThePrince prince) {
            if (fromTeenTamed) {
                prince.setTame(true);
                if (ownerUuid != null) {
                    prince.setOwnerUUID(ownerUuid);
                }
            }
            prince.set_ok_to_grow();
            return;
        }
        if ("ThePrince".equals(ent.getClass().getSimpleName())) {
            try {
                if (fromTeenTamed) {
                ...
                ent.getClass().getMethod("set_ok_to_grow").invoke(ent);
            } catch (ReflectiveOperationException ignored) {
            }
        }
    }

    private static void transferTameToAdult(Entity ent, UUID ownerUuid) {
