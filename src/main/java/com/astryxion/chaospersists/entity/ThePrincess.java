...
public class ThePrincess extends TamableAnimal {
    ...
    public InteractionResult mobInteract(Player par1EntityPlayer, InteractionHand hand) {
        ...
        if (this.isTame()
            ...
            if (!this.level().isClientSide) {
                ...
                par1EntityPlayer.displayClientMessage(
                        Component.literal("Princess fireballs extinguished."), true);
            }
            if (!par1EntityPlayer.getAbilities().instabuild) {
                var2.shrink(1);
                if (var2.isEmpty()) {
        ...
        if (this.isTame()
            ...
            if (!this.level().isClientSide) {
                ...
                this.setSpyroFire(1);
                par1EntityPlayer.displayClientMessage(
                        Component.literal("Princess fireballs lit!"), true);
            }
            if (!par1EntityPlayer.getAbilities().instabuild) {
                var2.shrink(1);
                if (var2.isEmpty()) {
                ...
                for (i = 0; i < 2; ++i) {
                    this.level()
                            .addParticle(
                                    ...
                                    (this.getRandom().nextGaussian() - this.getRandom().nextGaussian()) / 7.0
                                            + dm.x * 3.0,
                                    (this.getRandom().nextGaussian() - this.getRandom().nextGaussian()) / 7.0,
                                    (this.getRandom().nextGaussian() - this.getRandom().nextGaussian()) / 7.0
                                            + dm.z * 3.0);
    ...
    protected void customServerAiStep() {
        ...
        if (this.attack_level > 500) {
            if (this.getAttacking() != 0) {
                ...
                for (int i = 0; i < j; ++i) {
                    ...
                    if (ppwr instanceof PurplePower p) {
                        Vec3 princessMotion = this.getDeltaMovement();
                        p.setDeltaMovement(
                                princessMotion.x * 3.0, princessMotion.y * 3.0, princessMotion.z * 3.0);
                        p.setPurpleType(1 + this.getRandom().nextInt(3));
                    }
                }
            }
            ...
            if (this.level().getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING)) {
                for (int m = 0; m < 5; ++m) {
                    int i = this.getRandom().nextInt(5) - this.getRandom().nextInt(5);
                    int k = this.getRandom().nextInt(5) - this.getRandom().nextInt(5);
                    for (int j = -5; j < 5; ++j) {
                        BlockPos base =
                                new BlockPos(
            ...
            for (int m = 0; m < 2; ++m) {
                int i = this.getRandom().nextInt(4) - this.getRandom().nextInt(4);
                int k = this.getRandom().nextInt(4) - this.getRandom().nextInt(4);
        ...
        if (!this.isOrderedToSit() && !this.isInSittingPose()) {
            ...
            if (this.getRandom().nextInt(100) == 1) {
                ...
                // required onGround while flying, so after multi-kills they never landed.
    ...
    private boolean isSuitableTarget(LivingEntity par1EntityLiving, boolean par2) {
        ...
        return "EntityMosquito".equals(n);
    }
