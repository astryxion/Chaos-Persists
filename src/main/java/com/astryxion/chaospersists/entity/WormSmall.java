...
public class WormSmall extends MonsterEntity {
    public int upcount = 50;
    public int downcount = 0;

    public WormSmall(EntityType<? extends WormSmall> type, World par1World) {
        super(type, par1World);
        // EntityType registration: width=0.25f, height=1.0f
        this.xpReward = 0;
        this.noPhysics = true;
    }
    ...
    public static AttributeModifierMap createAttributes() {
        return MonsterEntity.createMonsterAttributes()
                .add(Attributes.MAX_HEALTH, ChaosPersists.WormSmall_stats.health)
                .add(Attributes.MOVEMENT_SPEED, 0.10000000149011612)
                .add(Attributes.ATTACK_DAMAGE, ChaosPersists.WormSmall_stats.attack)
                .build();
    }
        ...
        if (target != null || ChaosPersists.PlayNicely != 0) {
            if (this.upcount > 0) {
                ...
                if (bid != Blocks.AIR) {
                    ...
                    this.push(0.0, 0.15000000596046448, 0.0);
                ...
                Block bid = this.level.getBlockState(new BlockPos((int) this.getX(), (int) this.getY() + 2, (int) this.getZ())).getBlock();
                if (bid == Blocks.TALL_GRASS) {
                    bid = Blocks.AIR;
                }
                if (bid != Blocks.AIR) {
    ...
    public void tick() {
        ...
        super.tick();
        this.setDeltaMovement(this.getDeltaMovement().x, this.getDeltaMovement().y * 0.75, this.getDeltaMovement().z);
    }
    ...
    protected void customServerAiStep() {
        ...
        target = this.level.getNearestPlayer(this, 4.0);
