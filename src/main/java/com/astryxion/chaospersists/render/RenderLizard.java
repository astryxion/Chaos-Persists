...
extends LivingRenderer<Lizard, ModelLizard> {
    ...
    public RenderLizard(EntityRendererManager manager, ModelLizard par1Model, float par2, float par3) {
        super(manager, par1Model, par2 * par3);
        this.model = this.getModel();
        this.scale = par3;
    }
    @Override
    protected void scale(Lizard entity, MatrixStack matrixStack, float partialTick) {
        float s = this.scale;
    ...
    public ResourceLocation getTextureLocation(Lizard entity) {
        ...
        if ((c = entity).get_is_activated() != 0) {
            if (c.getHatColor() == 2) {
                return texture2;
            }
            if (c.getHatColor() == 3) {
