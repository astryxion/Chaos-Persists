...
public final class MyDispenserBehaviorDeadIrukandji implements IDispenseItemBehavior {
    @Override
    public ItemStack dispense(IBlockSource source, ItemStack stack) {
        World level = source.getLevel();
        IPosition position = DispenserBlock.getDispensePosition(source);
        Direction facing = source.getBlockState().getValue(DispenserBlock.FACING);
        DeadIrukandji projectile = new DeadIrukandji(level, position.x(), position.y(), position.z());
